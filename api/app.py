    ...
    try:
        ...
        for sys_col, aliases in system_columns.items():
            ...
            for col in columns:
                ...
                if col_lower in aliases or any(alias in col_lower for alias in aliases) or any(col_lower == alias.replace("_", " ") for alias in aliases):
                    sugestoes[sys_col] = col
                    found = True
                    break
            if not found and sys_col == "NOME_CURSO" and "NOME" in columns:
                sugestoes["NOME_CURSO"] = "NOME"
            if not found and sys_col == "ID_ALUNO" and "PESSOA" in columns:
